...
/* ─────────────────────────────────────────────────────────────
   PHOSPHOR ICONS (inline SVG — same set as rxresume)
───────────────────────────────────────────────────────────── */
const Ph = {
  Globe: ({ size = 15 }) => (
...
export default function Navbar() {
  ...
  return (
	<>
	  {/* ── NAVBAR ── */}
	  <nav className={`pro-navbar${scrolled ? ' scrolled' : ''}`} role="navigation" aria-label="Main navigation">
		<div className="navbar-container">

		  {/* Brand */}
		  <Link to="/" className="nav-brand" aria-label={t?.siteName ?? 'Mamey'}>
			<img src="/mamey.png" alt="" aria-hidden="true" />
		  ...
		  {/* Desktop nav */}
		  <div className="nav-desktop" role="menubar">
			{navItems.map((item) => (
			  <Link
				key={item.path}
				to={item.path}
		  ...
		  <div className="nav-actions">
			...
			>
			  <Ph.Globe size={14} />
			  {t?.switchLang ?? (lang === 'en' ? 'العربية' : 'English')}
			</button>
	  ...
	  >
		...
		<div className="drawer-header">
		  <Link to="/" className="nav-brand" onClick={() => setDrawerOpen(false)}>
			<img src="/mamey.png" alt="" aria-hidden="true" style={{ width: 32 }} />
			<div className="nav-brand-text">
			  <div className="nav-brand-name" style={{ fontSize: '1.1rem' }}>
				{t?.siteName ?? 'MAMEY'}
			  </div>
			  <div className="nav-brand-sub">{t?.siteTagline ?? 'General Trading & Investment'}</div>
			</div>
